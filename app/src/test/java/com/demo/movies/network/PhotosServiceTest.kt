...
class PhotosServiceTest {
    ...
    @Test
    fun getPhotosList_canBeParsed() = runBlocking {
        ...
        val photosResponse = service.getPhotos("Blockers",1, FLICKER_API_KEY)
        photosResponse.body()?.photosList?.photosList?.isEmpty()?.let { assertFalse(it) }

        mockWebServer.shutdown()
    }

    /*Test failure status on case sent invalid API KEY*/
    @Test
    fun getWrongStat_invalidApiKey() = runBlocking {
        mockWebServer = MockWebServer()
        mockWebServer.enqueue(MockResponse().setBody(mockResponseAllPhotos))
        mockWebServer.start()
        ...
        val photosResponse = service.getPhotos("Blockers",1, "123")

        photosResponse.isSuccessful.let { assertTrue(it && photosResponse.body()?.stat =="fail") }

        mockWebServer.shutdown()
    }

    /*Test failure status on case sent invalid title*/
    @Test
    fun getWrongStat_invalidTitle() = runBlocking {
        mockWebServer = MockWebServer()
        mockWebServer.enqueue(MockResponse().setBody(mockResponseAllPhotos))
        mockWebServer.start()
        ...
        val photosResponse = service.getPhotos("",1, FLICKER_API_KEY)
